...
/*
	...
	(1) 클라이언트가 호스트에게 원하는 웹 페이지를 보여달라고 요청한다,
		...
		naver.com - IP 주소 (또는 도메인)
		/new - 사용자가 해당 웹 서버에서 특별히 원하는 자원 (웹 페이지 또는 그림 등);
	
	(2) 사용자의 요청이 Apache-Tomcat에 도착한다 
		Apache-Tomcat에 등록되어있는 URL 매핑들 중 일치하는 자원을 찾는다.
		
		등록할 수 있는 자원들 - .jsp, .java(Servlet), .html, 
						 기타 자원들(.js, .css, .jpg ..)
		등록 방법들 - @WebServlet 어노테이션, webapp(webContent) 밑에 직접 넣기.. 등
	
	(3) 등록된 자원들 중 사용자의 요청에 따라 페이지가 다른 결과들이 나오는 페이지를 동적 웹 페이지라고 한다.
