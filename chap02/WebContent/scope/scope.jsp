...
<body>
	...
	<dl>
		<dt>page</dt>
		<dd>페이지 스코프에 설정된 어트리뷰트는 지역변수처럼 해당 페이지(서블릿)를 벗어나면 사라진다</dd>
		<dt>request</dt>
		<dd>리퀘스트 스코프에 설정된 어트리뷰트는 응답이 발생하기 전까지 수명이 유지된다</dd>
		<dt>session</dt>
		<dd>
			사용자의 세션이 만료될때까지 수명이 유지된다. 세션은 브라우저별로 존재한다.
			IP가 같더라도 다른 브라우저를 사용하면 서버로부터 새로운 세션을 받아갈 수 있다.
		</dd>
		<dt>application</dt>
		<dd>어플리케이션 스코프에 저장하는 어트리뷰트는 서버가 꺼질때까지 유지된다.(jsp의 전역변수 static)</dd>
	</dl>
	...
	<ol>
		...
		<li>사용자의 웹 브라우저에 첫 번째 응답과 세션ID가 함께 도착한다.</li>
		<li>
			해당 웹서버에 다시 요청을 보낼때 브라우저에 저장된 세션ID가 함께전송된다
			웹 서버는 해당 세션ID를 이용해 사용자를 식별한다.
		</li>
		<li>세션 ID는 사용자의 웹 브라우저가 종료될때까지 유지된다.</li>
		<li>세션 어트리뷰트에 저장하는 자바 객체들은 사용자의 세션이 만료될때까지 살아있게 된다.</li>
